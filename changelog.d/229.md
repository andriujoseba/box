...
- A daemon that will not list projects no longer reads as a host with no granted users: `setup-host` says which projects it could not check and withholds `Host ready`, and `box doctor` reports the project list as unread rather than certifying a contract it never looked at. A listing that emits a row and then fails counts as unread too — a partial answer is not the host's inventory, and neither tool now reports on the projects it did not reach (#229).
